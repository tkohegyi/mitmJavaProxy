...
public class MailingListIssuesTest extends DummyServerTest {
    ...
    public void coreTestThatInterceptorIsCalled() throws IOException, InterruptedException {
        ...
        Assertions.assertTrue(body.contains("this is a.txt"));
        Assertions.assertTrue(interceptorHit[0]);
    }
    ...
    public void testThatInterceptorCanCaptureCallingIpAddress() throws IOException, InterruptedException {
        ...
        Assertions.assertTrue(body.contains("this is a.txt"));
        Assertions.assertEquals("127.0.0.1", remoteHost[0], "Remote host incorrect");
    }
    ...
    public void testThatInterceptorsCanRewriteUrls() throws IOException, InterruptedException {
        ...
        Assertions.assertTrue(body.contains("this is b.txt"));
    }
    ...
    public void testThatProxyCanCaptureBodyInHar() throws IOException, InterruptedException {
        ...
        Assertions.assertNotNull(har, "Har is null");
        HarLog log = har.getLog();
        Assertions.assertNotNull(log, "Log is null");
        List<HarEntry> entries = log.getEntries();
        Assertions.assertNotNull(entries, "Entries are null");
        HarEntry entry = entries.get(0);
        Assertions.assertNotNull(entry, "No entry found");
        HarResponse response = entry.getResponse();
        Assertions.assertNotNull(response, "Response is null");
        HarContent content = response.getContent();
        Assertions.assertNotNull(content, "Content is null");
        String mime = content.getMimeType();
        Assertions.assertEquals( "text/plain", mime, "Mime not matched");
        String text = content.getText();
        Assertions.assertEquals( "this is a.txt", text, "Text not matched");
    }
    ...
    public void testThatProxyCanCaptureJsonRpc() throws IOException, InterruptedException {
        ...
        Assertions.assertTrue(body.contains("{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{}}"));
        ...
        Assertions.assertNotNull(postdata, "PostData is null");
        Assertions.assertTrue(postdata.getText().contains("{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"test\",\"params\":{}}"));
    }
    ...
    public void testThatTraditionalPostParamsAreCaptured() throws IOException, InterruptedException {
        ...
        HarPostData postdata = request.getPostData();
        Assertions.assertNotNull(postdata, "PostData is null");
        Assertions.assertEquals("application/x-www-form-urlencoded", postdata.getMimeType());
        Assertions.assertEquals(1, postdata.getParams().size());
        Assertions.assertEquals("foo", postdata.getParams().get(0).getName());
        Assertions.assertEquals("bar", postdata.getParams().get(0).getValue());
    }
    ...
    public void testThatImagesAreCapturedAsBase64EncodedContent() throws IOException, InterruptedException {
        ...
        Assertions.assertTrue( Arrays.equals(o1.toByteArray(), o2.toByteArray()), "Image does not match file system");
        ...
        Assertions.assertNotNull(content, "Content is null");
        String mime = content.getMimeType();
        Assertions.assertEquals( "image/png", mime, "Mime not matched");
        String text = content.getText();
        ...
        Assertions.assertEquals( base64, text, "Base64 not correct");
    }
