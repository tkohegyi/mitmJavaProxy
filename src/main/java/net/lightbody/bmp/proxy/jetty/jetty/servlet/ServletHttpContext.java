...
/**
 * ServletHttpContext.
 * Extends HttpContext with conveniance methods for adding servlets. Enforces a single ServletHandler per context.
 *
...
public class ServletHttpContext extends HttpContext {
     ...
     * Conveniance method. If no ServletHandler exists, a new one is added to the context.
     *
    ...
    /**
     * Add a servlet to the context. Conveniance method.
     ...
     */
    public synchronized ServletHolder addServlet(String pathSpec, String className)
     ...
     * @throws IllegalAccessException
     */
    public synchronized ServletHolder addServlet(String name, String pathSpec, String className)
            throws ClassNotFoundException, InstantiationException, IllegalAccessException {
    ...
    protected boolean jSecurityCheck(String pathInContext, HttpRequest request, HttpResponse response) throws IOException {
        if (getAuthenticator() instanceof FormAuthenticator &&
                ...
                getAuthenticator().authenticate(getRealm(), pathInContext, request, response) == null) {
            return false;
        }
        return true;
    }
     ...
     * looked up in the map of encodings. If no encoding is found, then the locale language is looked up.
     *
