...
public class LogImpl implements org.apache.commons.logging.Log {
    ...
    public synchronized void add(String logSinkClass) {
        try {
            if (logSinkClass == null || logSinkClass.length() == 0)
                logSinkClass = "net.lightbody.bmp.proxy.jetty.log.OutputStreamLogSink";
    ...
    private synchronized void defaultInit() {
        if (!_initialized) {
            _initialized = true;
            String sinkClasses = System.getProperty("LOG_SINKS", "net.lightbody.bmp.proxy.jetty.log.OutputStreamLogSink");
            ...
            while (sinkTokens.hasMoreTokens()) {
                ...
                try {
                    ...
                    if (net.lightbody.bmp.proxy.jetty.log.LogSink.class.isAssignableFrom(sinkClass)) {
                        ...
                        System.err.println(sinkClass + " is not a net.lightbody.bmp.proxy.jetty.log.LogSink");
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
