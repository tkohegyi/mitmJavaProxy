 ...
 * <LI>Notification of significant output events for filter triggering, header flushing, etc.
 * </UL>
 * <p>
...
public class HttpOutputStream extends OutputStream implements OutputObserver, HttpMessage.HeaderWriter {
    ...
    private final Logger log = LoggerFactory.getLogger(HttpOutputStream.class);

    private final int _headerReserve;
    private final OutputStream _realOut;
     ...
     * Note that this is the minimal buffer size and that installed filters may perform their own buffering 
     * and are likely to change the size of the output. Also the pre and post reserve buffers may be
     ...
     * If no data has been committed, the buffer output is discarded and the filters may be reinitialized.
     *
    ...
    /**
     * Add an Output Observer.
     * Output Observers get notified of significant events on the output stream. Observers are called 
     * in the reverse order they were added. They are removed when the stream is closed.
     *
     * @param observer The observer.
    ...
    /**
     * Null the output.
     * All output written is discarded until the stream is reset. Used for HEAD requests.
     */
    ...
    /**
     * Output Notification.
     * Called by the internal Buffered Output and the event is passed on to this streams observers.
     */
