...
public class HttpRequest extends HttpMessage {
     ...
     */
    public static int __maxFormContentSize = Integer.getInteger("net.lightbody.bmp.proxy.jetty.http.HttpRequest.maxFormContentSize", 200000);
    ...
    private static final Cookie[] __noCookies = new Cookie[0];
     ...
     *
     * @return True if the request has been set to handled or the associated response is not editable.
     */
    ...
    /**
     * Get the HTTP method for this request. Returns the method with which the request was made.
     * The returned value can be "GET", "HEAD", "POST", or an extension method. Same as the CGI variable REQUEST_METHOD.
     *
     * @return The method
     */
    ...
    /**
     * Get the request Scheme. The scheme is obtained from an absolute URI. If the URI in the request is not absolute,
     * then the connections default scheme is returned. If there is no connection "http" is returned.
     *
     ...
     *
     * @return The host name obtained from an absolute URI, the HTTP header field, the requests connection or the local host name.
     */
    ...
    /**
     * Get the request port. The port is obtained either from an absolute URI, the HTTP Host header field,
     * the connection or the default.
     *
    ...
    /**
     * Force a removeField. This call ignores the message state and forces a field to be removed from the request.
     ...
     * extension transfer codings in quality order. An empty list implies that only "chunked" is acceptable.
     * A null list implies that no transfer coding can be applied.
     * <p>
     * If the "trailer" coding is found in the TE field, then message trailers are enabled in any linked response.
     *
    ...
    /**
     * Extract Parameters from query string and/or form content.
     */
    private void extractParameters() {
        ...
        if (_state == __MSG_RECEIVED) {
            ...
            if (content_type != null && content_type.length() > 0) {
                ...
                if (HttpFields.__WwwFormUrlEncode.equalsIgnoreCase(content_type) && HttpRequest.__POST.equals(getMethod())) {
                    ...
                    if (content_length == 0) {
                        ...
                        try {
                            ...
                            // Read the content
                            ByteArrayOutputStream2 bout = new ByteArrayOutputStream2(max > 0 ? max : 4096);
                        ...
                        if (n.startsWith("$")) {
                            if ("$version".equalsIgnoreCase(n)) {
                                ...
                                if ("$path".equalsIgnoreCase(n) && cookie != null) {
                                    ...
                                    if ("$domain".equalsIgnoreCase(n) && cookie != null) {
                                        cookie.setDomain(v);
                                    }
                                }
                            }
