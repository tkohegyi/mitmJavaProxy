 ...
 * <p>
 * Note that this wrapper is not synchronized and if a request is to be operated on by multiple threads,
 * then higher level synchronizations may be required.
 *
...
public class ServletHttpRequest implements HttpServletRequest {
     ...
     *
     * @param request
     * @return The core ServletHttpRequest which must be the underlying request object
    ...
    /**
     * Set servletpath and pathInfo. Called by the Handler before passing a request to a particular holder
     * to split the context path into a servlet path and path info.
     *
     * @param servletPath
     * @param pathInfo
     */
    ...
    public Locale getLocale() {
        Enumeration enm = _httpRequest.getFieldValues(HttpFields.__AcceptLanguage, HttpFields.__separators);
    ...
    public String getAuthType() {
        ...
        if (at.equals(SecurityConstraint.__BASIC_AUTH)) {
            return HttpServletRequest.BASIC_AUTH;
        }
        if (at.equals(SecurityConstraint.__FORM_AUTH)) {
            return HttpServletRequest.FORM_AUTH;
        }
        if (at.equals(SecurityConstraint.__DIGEST_AUTH)) {
            return HttpServletRequest.DIGEST_AUTH;
        }
        if (at.equals(SecurityConstraint.__CERT_AUTH)) {
            return HttpServletRequest.CLIENT_CERT_AUTH;
        }
        if (at.equals(SecurityConstraint.__CERT_AUTH2)) {
            return HttpServletRequest.CLIENT_CERT_AUTH;
        }
        return at;
    }
    ...
    void setRequestedSessionId(String pathParams) {
            ...
            if (cookies != null && cookies.length > 0) {
                for (int i = 0; i < cookies.length; i++) {
                    if (SessionManager.__SessionCookie.equalsIgnoreCase(cookies[i].getName())) {
                        ...
                        log.debug("Got Session {} from cookie", _requestedSessionId);
                    }
                }
            }
        }
        ...
        if (pathParams != null && pathParams.startsWith(SessionManager.__SessionURL)) {
            ...
            log.debug("Got Session {} from URL", id);

            if (_requestedSessionId == null) {
                _requestedSessionId = id;
                _sessionIdState = __SESSIONID_URL;
            } else {
                if (!id.equals(_requestedSessionId))
    ...
    public boolean isRequestedSessionIdFromCookie() {
        return _sessionIdState.equals(__SESSIONID_COOKIE);
    }

    public boolean isRequestedSessionIdFromURL() {
        return _sessionIdState.equals(__SESSIONID_URL);
    }
