 ...
 * The behaviour of the listener can be controlled with the attributues of the ThreadedServer and ThreadPool 
 * from which it is derived. Specifically: <PRE>
...
public class SocketListener extends ThreadedServer implements HttpListener {
    private final Logger log = LoggerFactory.getLogger(SocketListener.class);
    ...
    /**
     * @param identifyListener If true, the listener name is added to all requests as the org.mortbay.http.HttListener attribute
     */
    ...
    /**
     * @param lowResources The number of idle threads needed to not be in low resources state.
     */
     ...
     * When the listener is low on resources, this timeout is used for idle persistent connections.
     * It is desirable to have this set to a short period of time so that idle persistent connections do not consume
     * resources on a busy server.
     *
     * @param ms time in ms that connections will persist if listener is low on resources.
     */
     ...
     * This method extracts the socket from the connection and calls the customizeRequest(Socket,HttpRequest) method.
     *
     * @param request
     */
     ...
     * Derived versions of SocketListener may specialize this method to customize the request with attributes
     * of the socket used (eg SSL session ids). This version resets the SoTimeout if it has been reduced due to
     * low resources.  Derived implementations should call super.customizeRequest(socket,request)
     * unless persistConnection has also been overridden and not called.
     *
     * @param request
     */
     ...
     * This method is called by the HttpConnection in order to prepare a connection to be persisted.
     * For this implementation, if the listener is low on resources, the connection read timeout is set
     * to lowResourcePersistTimeMs.  The customizeRequest method is used to reset this to the normal
